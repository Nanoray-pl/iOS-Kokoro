...
public struct AnyProxy<EnclosingSelf, Value> {
	...
	public init<KeyPathValue>(_ keyPath: ReferenceWritableKeyPath<EnclosingSelf, KeyPathValue>, get getMapper: @escaping (KeyPathValue) -> Value, set setMapper: @escaping (Value) -> KeyPathValue, didSet didSetClosure: DidSetWithoutOldValueClosure? = nil) {
		self.init(
	...
	public init<KeyPathValue>(_ keyPath: ReferenceWritableKeyPath<EnclosingSelf, KeyPathValue>, get getMapper: @escaping (KeyPathValue) -> Value, set setMapper: @escaping (Value) -> KeyPathValue, didSet didSetClosure: DidSetWithOldValueClosure?) {
		self.init(
			getter: { getMapper($0[keyPath: keyPath]) },
			setter: { $0[keyPath: keyPath] = setMapper($1) },
			didSet: didSetClosure
